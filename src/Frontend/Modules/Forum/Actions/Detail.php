...
class Detail extends FrontendBaseBlock
{
    ...
    private function getData()
    {
        ...
        if ($this->URL->getParameter('revision', 'int') != 0) {
            // get data
            $this->record = FrontendForumModel::getRevision(
            ...
            // get by URL
            $this->record = FrontendForumModel::get($this->URL->getParameter(1));
        }
        ...
        // get comments
        $this->comments = FrontendForumModel::getComments($this->record['id']);

        // get tags
        $this->record['tags'] = FrontendTagsModel::getForItem('Forum', $this->record['id']);

        // get settings
        $this->settings = FrontendModel::getModuleSettings('Forum');

        // overwrite URLs
        $this->record['category_full_url'] = FrontendNavigation::getURLForBlock('Forum', 'Category') .
                                             '/' . $this->record['category_url'];
        $this->record['full_url'] = FrontendNavigation::getURLForBlock('Forum', 'Detail') . '/' . $this->record['url'];
...
//        $pd->setNoMarkup();
        ...
        $rssCommentsLink = FrontendNavigation::getURLForBlock('Forum', 'PostCommentsRss') .
                           '/' . $this->record['url'];
        ...
        if (isset($this->record['image']) && $this->record['image'] != '') {
            $this->header->addOpenGraphImage(
                FRONTEND_FILES_URL . '/forum/images/source/' . $this->record['image']
            );
        }
        ...
        $this->header->addOpenGraphData('type', 'post', true);
        ...
        // Twitter Card
        $imgURL = FRONTEND_FILES_URL . '/forum/images/source/' . $this->record['image'];
        ...
        if (count(FrontendForumModel::getAllCategories()) > 1) {
            $this->breadcrumb->addElement(
                $this->record['category_title'],
                FrontendNavigation::getURLForBlock('Forum', 'Category') . '/' . $this->record['category_url']
            );
        }
        ...
        // options
        if ($commentCount > 1) {
            $this->tpl->assign('forumCommentsMultiple', true);
        }
        ...
        $navigation = FrontendForumModel::getNavigation($this->record['id']);
    ...
    private function validateForm()
    {
        ...
        if ($this->frm->isSubmitted()) {
            ...
            // does the key exists?
            if (\SpoonSession::exists('forum_comment_' . $this->record['id'])) {
                // calculate difference
                $diff = time() - (int) \SpoonSession::get('forum_comment_' . $this->record['id']);
            ...
            if ($this->frm->isCorrect()) {
                ...
                if ($moderationEnabled) {
                    ...
                    if (!FrontendForumModel::isModerated($author, $email)) {
                ...
                // insert comment
                $comment['id'] = FrontendForumModel::insertComment($comment);

                // trigger event
                FrontendModel::triggerEvent('Forum', 'after_add_comment', array('comment' => $comment));
                ...
                // notify the admin
                FrontendForumModel::notifyAdmin($comment);

                // store timestamp in session so we can block excessive usage
                \SpoonSession::set('forum_comment_' . $this->record['id'], time());
