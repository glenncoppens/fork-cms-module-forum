...
class Archive extends FrontendBaseBlock
{
    ...
    private function getData()
    {
        ...
        if ($this->month !== null && mb_strlen($this->month) != 2) {
            ...
            $this->redirect(
                FrontendNavigation::getURLForBlock('Forum', 'Archive') . '/' . $this->year . '/' . str_pad(
                    $this->month,
                    2,
                    '0',
                    STR_PAD_LEFT
        ...
        // set URL and limit
        $this->pagination['url'] = FrontendNavigation::getURLForBlock('Forum', 'Archive') . '/' . $url;
        $this->pagination['limit'] = FrontendModel::getModuleSetting('Forum', 'overview_num_items', 10);

        // populate count fields in pagination
        $this->pagination['num_items'] = FrontendForumModel::getAllForDateRangeCount($this->startDate, $this->endDate);
        ...
        // get posts
        $this->items = FrontendForumModel::getAllForDateRange(
            $this->startDate,
            $this->endDate,
    ...
    private function parse()
    {
        ...
        $this->tpl->assign('allowComments', FrontendModel::getModuleSetting('Forum', 'allow_comments'));
