				...
				<div class="inner">
					<header class="hd">
						...
						<ul>
							...
							<li>
								{option:!items.comments}<a href="{$items.full_url}#{$actComment}">{$msgForumNoComments|ucfirst}</a>{/option:!items.comments}
								{option:items.comments}
									{option:items.comments_multiple}<a href="{$items.full_url}#{$actComments}">{$msgForumNumberOfComments|sprintf:{$items.comments_count}}</a>{/option:items.comments_multiple}
									{option:!items.comments_multiple}<a href="{$items.full_url}#{$actComments}">{$msgForumOneComment}</a>{/option:!items.comments_multiple}
								{/option:items.comments}
							</li>
							{/option:items.allow_comments}
						</ul>
					...
					<div class="bd content">
						{option:items.image}<img src="{$FRONTEND_FILES_URL}/forum/images/source/{$items.image}" alt="{$items.title}" />{/option:items.image}
