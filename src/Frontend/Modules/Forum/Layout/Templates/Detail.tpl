...
{*
<div id="forumDetail">
	<post class="mod post" itemscope itemtype="http://schema.org/Forum">
		<div class="inner">
			...
			<header class="hd">
				...
				<ul>
					<li>
						...
						{* Category*}
						{$lblIn} {$lblThe} {$lblCategory} <a itemprop="postSection" href="{$item.category_full_url}" title="{$item.category_title}">{$item.category_title}</a>{option:!item.tags}.{/option:!item.tags}
					...
					{option:item.allow_comments}
						<li>
							{option:!comments}<a href="{$item.full_url}#{$actComment}" itemprop="discussionUrl">{$msgForumNoComments|ucfirst}</a>{/option:!comments}
							{option:comments}
								{option:forumCommentsMultiple}<a href="{$item.full_url}#{$actComments}" itemprop="discussionUrl">{$msgForumNumberOfComments|sprintf:{$commentsCount}}</a>{/option:forumCommentsMultiple}
								{option:!forumCommentsMultiple}<a href="{$item.full_url}#{$actComments}" itemprop="discussionUrl">{$msgForumOneComment}</a>{/option:!forumCommentsMultiple}
							{/option:comments}
						</li>
					{/option:item.allow_comments}
					<li>
			...
			<div class="bd content" itemprop="postBody">
				{option:item.image}<img src="{$FRONTEND_FILES_URL}/forum/images/source/{$item.image}" alt="{$item.title}" itemprop="image" />{/option:item.image}
				{$item.text}
			</div>
			<footer class="ft">
				{option:navigation}
				<ul class="pageNavigation">
					{option:navigation.previous}
						<li class="previousLink">
							<a href="{$navigation.previous.url}" rel="prev">{$lblPreviousPost|ucfirst}: {$navigation.previous.title}</a>
						</li>
					{/option:navigation.previous}
					{option:navigation.next}
						<li class="nextLink">
							<a href="{$navigation.next.url}" rel="next">{$lblNextPost|ucfirst}: {$navigation.next.title}</a>
						</li>
					{/option:navigation.next}
				</ul>
				{/option:navigation}
	...
	{option:comments}
		{option:item.allow_comments}
			<section id="forumComments" class="mod" itemscope itemtype="http://schema.org/Post">
				<div class="inner">
					<header class="hd">
	...
	{/option:comments}
	{option:item.allow_comments}
		<section id="forumCommentForm" class="mod">
			<div class="inner">
				<header class="hd">
					<h3 id="{$actComment}">{$msgComment|ucfirst}</h3>
				</header>
				<div class="bd">
					{option:commentIsInModeration}<div class="message warning"><p>{$msgForumCommentInModeration}</p></div>{/option:commentIsInModeration}
					{option:commentIsSpam}<div class="message error"><p>{$msgForumCommentIsSpam}</p></div>{/option:commentIsSpam}
					{option:commentIsAdded}<div class="message success"><p>{$msgForumCommentIsAdded}</p></div>{/option:commentIsAdded}
					{form:commentsForm}
