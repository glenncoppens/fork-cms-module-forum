...
class Model implements FrontendTagsInterface
{
    ...
    public static function get($URL)
    {
        $return = (array) FrontendModel::getContainer()->get('database')->getRecord(
            'SELECT i.id, i.revision_id, i.title, i.text, i.url,
             ...
             WHERE i.status = ? AND i.hidden = ? AND i.publish_on <= ? AND i.url = ?
             LIMIT 1',
            array('active', 'N', FrontendModel::getUTCDate('Y-m-d H:i') . ':00', (string) $URL)
        );
    ...
    public static function getAll($limit = 10, $offset = 0)
    {
        $items = (array) FrontendModel::getContainer()->get('database')->getRecords(
            'SELECT i.id, i.revision_id, i.title, i.text, i.url, i.num_comments AS comments_count,
             UNIX_TIMESTAMP(i.publish_on) AS publish_on, i.profile_id, i.allow_comments
             FROM forum_posts AS i
             WHERE i.status = ? AND i.hidden = ? AND i.publish_on <= ?
        ...
        // init var
        $link = FrontendNavigation::getURLForBlock('Forum', 'Detail');
        ...
        foreach ($items as $key => $row) {
            ...
            if (!FrontendModel::getModuleSetting('Forum', 'allow_comments')) {
                $items[$key]['allow_comments'] = false;
            }
        }

        // get all tags
        $tags = FrontendTagsModel::getForMultipleItems('Forum', array_keys($items));
    ...
    public static function getAllCategories()
    {
        $return = (array) FrontendModel::getContainer()->get('database')->getRecords(
            'SELECT c.id, c.title AS label, m.url, COUNT(c.id) AS total, m.data AS meta_data
             ...
             INNER JOIN forum_posts AS i ON c.id = i.category_id AND c.language = i.language
    ...
    {
        $items = (array) FrontendModel::getContainer()->get('database')->getRecords(
            'SELECT i.id, i.revision_id, i.language, i.title, i.text, i.num_comments AS comments_count,
        ...
        // init var
        $link = FrontendNavigation::getURLForBlock('Forum', 'Detail');
        $categoryLink = FrontendNavigation::getURLForBlock('Forum', 'Category');
        $folders = FrontendModel::getThumbnailFolders(FRONTEND_FILES_PATH . '/Forum/Images', true);
    ...
    public static function getArchiveNumbers()
    {
        ...
        // init vars
        $stats = array();
        $link = FrontendNavigation::getURLForBlock('Forum', 'Archive');
    ...
    public static function getNavigation($id)
    {
        ...
        // init var
        $navigation = array();
        $detailLink = FrontendNavigation::getURLForBlock('Forum', 'Detail') . '/';
    ...
    public static function getRecentComments($limit = 5)
    {
        ...
        $comments = (array) FrontendModel::getContainer()->get('database')->getRecords(
            'SELECT c.id, c.author, c.website, c.email, UNIX_TIMESTAMP(c.created_on) AS created_on, c.text,
             ...
             m.url AS post_url
             FROM forum_comments AS c
             INNER JOIN forum_posts AS i ON c.post_id = i.id AND c.language = i.language
             INNER JOIN meta AS m ON i.meta_id = m.id
    ...
    public static function getRelated($id, $limit = 5)
    {
        ...
        // get the related IDs
        $relatedIDs = (array) FrontendTagsModel::getRelatedItemsByTags($id, 'Forum', 'Forum', $limit);
     ...
     * @return int
     */
    public static function insertPost(array $item)
    {
    ...
    public static function notifyAdmin(array $comment)
    {
        ...
        // get settings
        $notifyByMailOnComment = FrontendModel::getModuleSetting('Forum', 'notify_by_email_on_new_comment', false);
        ...
        // create URLs
        $URL = SITE_URL . FrontendNavigation::getURLForBlock('Forum', 'Detail') . '/' .
               $comment['post_url'] . '#comment-' . $comment['id'];
        $backendURL = SITE_URL . FrontendNavigation::getBackendURLForBlock('comments', 'Forum') . '#tabModeration';
     ...
     * @return string
     */
    public static function getUniqueUrl($URL, $id = null)
    {
        $URL = (string) $URL;
    ...
    {
        $items = (array) FrontendModel::getContainer()->get('database')->getRecords(
            'SELECT i.id, i.title, i.text, m.url
             FROM forum_posts AS i
             INNER JOIN meta AS m ON i.meta_id = m.id
        ...
        foreach ($items as &$item) {
            $item['full_url'] = FrontendNavigation::getURLForBlock('Forum', 'Detail') . '/' . $item['url'];
        }
