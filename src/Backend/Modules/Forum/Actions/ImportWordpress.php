...
use Backend\Modules\Forum\Engine\Model;

/**
 * This import-action will let you import a wordpress forum
 *
...
class ImportWordpress extends BackendBaseActionEdit
{
    ...
    private function processAttachment($xml)
    {
        ...
        // Set paths
        $imagesPath = FRONTEND_FILES_PATH . '/userfiles/images/forum';
        $imagesURL = FRONTEND_FILES_URL . '/userfiles/images/forum';
     ...
     *
     * We'll check if the category exists in the fork forum module, and create it if it doesn't.
     *
    ...
    private function handleCategory($category = '')
    {
        ...
        $db = BackendModel::getContainer()->get('database');
        $id = (int) $db->getVar(
            'SELECT id FROM forum_categories WHERE title=? AND language=?',
