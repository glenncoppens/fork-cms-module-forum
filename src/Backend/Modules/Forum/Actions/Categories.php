...
/**
 * This is the categories-action, it will display the overview of forum categories
 *
...
class Categories extends BackendBaseActionIndex
{
    ...
    private function loadDataGrid()
    {
        ...
        $this->dataGrid = new BackendDataGridDB(
            BackendForumModel::QRY_DATAGRID_BROWSE_CATEGORIES,
    ...
    public static function setClickableCount($count, $link)
    {
        ...
        if ($count > 1) {
            $return = '<a href="' . $link . '">' . $count . ' ' . BL::getLabel('Posts') . '</a>';
        } elseif ($count == 1) {
            $return = '<a href="' . $link . '">' . $count . ' ' . BL::getLabel('Post') . '</a>';
        }
