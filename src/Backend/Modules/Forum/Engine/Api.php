...
class Api
{
    ...
    public static function commentsDelete($id)
    {
        ...
        if (BaseAPI::isAuthorized() && BaseAPI::isValidRequestMethod('POST')) {
            ...
            // update statuses
            BackendForumModel::deleteComments($id);
        }
    }
    ...
    public static function commentsGet($status = null, $limit = 30, $offset = 0)
    {
        ...
        if (BaseAPI::isAuthorized() && BaseAPI::isValidRequestMethod('GET')) {
            ...
            foreach ($comments as $row) {
                ...
                // post meta data
                $item['comment']['post']['@attributes']['id'] = $row['post_id'];
                $item['comment']['post']['@attributes']['lang'] = $row['post_language'];
                $item['comment']['post']['title'] = $row['post_title'];
                $item['comment']['post']['url'] = SITE_URL .
                    BackendModel::getURLForBlock('Forum', 'Detail', $row['post_language']) . '/' . $row['post_url']
                ;
                ...
                $item['comment']['url'] = $item['comment']['post']['url'] . '#comment-' . $row['id'];
    ...
    public static function commentsGetById($id)
    {
        ...
        if (BaseAPI::isAuthorized() && BaseAPI::isValidRequestMethod('GET')) {
            // get comment
            $comment = (array) BackendForumModel::getComment($id);
            ...
            // post meta data
            $item['comment']['post']['@attributes']['id'] = $comment['post_id'];
            $item['comment']['post']['@attributes']['lang'] = $comment['language'];
            $item['comment']['post']['title'] = $comment['post_title'];
            $item['comment']['post']['url'] = SITE_URL .
                BackendModel::getURLForBlock('Forum', 'Detail', $comment['language']) . '/' . $comment['post_url']
            ;
            ...
            $item['comment']['url'] = $item['comment']['post']['url'] . '#comment-' . $comment['id'];
    ...
    public static function commentsUpdate(
        ...
        if (BaseAPI::isAuthorized() && BaseAPI::isValidRequestMethod('POST')) {
            ...
            if ($text !== null || $authorName !== null || $authorEmail != null || $authorWebsite !== null) {
                ...
                // update the comment
                BackendForumModel::updateComment($item);
            }

            // change the status if needed
            if ($status !== null) {
                BackendForumModel::updateCommentStatuses(array($id), $status);
            }
        }
    }
    ...
    public static function commentsUpdateStatus($id, $status)
    {
        ...
        if (BaseAPI::isAuthorized() && BaseAPI::isValidRequestMethod('POST')) {
            ...
            // update statuses
            BackendForumModel::updateCommentStatuses($id, $status);
        }
    }
}
