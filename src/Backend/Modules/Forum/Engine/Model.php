...
class Model
{
    const QRY_DATAGRID_BROWSE =
        'SELECT i.hidden, i.id, i.revision_id, i.title, p.display_name, 
         UNIX_TIMESTAMP(i.publish_on) AS publish_on, i.profile_id, i.num_comments AS comments
         FROM forum_posts AS i
         INNER JOIN profiles AS p ON p.id = i.profile_id
         WHERE i.status = ?';

    const QRY_DATAGRID_BROWSE_CATEGORIES =
        'SELECT i.id, i.title, COUNT(p.id) AS num_items
         FROM forum_categories AS i
         LEFT OUTER JOIN forum_posts AS p ON i.id = p.category_id AND p.status = ? AND p.language = i.language
         WHERE i.language = ?
         GROUP BY i.id';
    ...
    const QRY_DATAGRID_BROWSE_DRAFTS =
        'SELECT i.id, i.profile_id, i.revision_id, i.title, UNIX_TIMESTAMP(i.edited_on) AS edited_on, i.num_comments AS comments
         FROM forum_posts AS i
         INNER JOIN
         (
             SELECT MAX(i.revision_id) AS revision_id
             FROM forum_posts AS i
             WHERE i.status = ? AND i.profile_id = ? AND i.language = ?
             GROUP BY i.id
         ) AS p
    ...
    const QRY_DATAGRID_BROWSE_DRAFTS_FOR_CATEGORY =
        'SELECT i.id, i.profile_id, i.revision_id, i.title, UNIX_TIMESTAMP(i.edited_on) AS edited_on, i.num_comments AS comments
         ...
         (
             SELECT MAX(i.revision_id) AS revision_id
             FROM forum_posts AS i
             WHERE i.category_id = ? AND i.status = ? AND i.profile_id = ? AND i.language = ?
             GROUP BY i.id
         ) AS p
         WHERE i.revision_id = p.revision_id';

    const QRY_DATAGRID_BROWSE_RECENT =
        'SELECT i.id, i.revision_id, i.title, UNIX_TIMESTAMP(i.edited_on) AS edited_on, i.profile_id, i.num_comments AS comments
         FROM forum_posts AS i
    ...
    public static function checkSettings()
    {
        ...
        if (BackendAuthentication::isAllowedAction('Settings', 'Forum')) {
            // rss title
            if (BackendModel::getModuleSetting('Forum', 'rss_title_' . BL::getWorkingLanguage(), null) == '') {
                $warnings[] = array(
                    'message' => sprintf(
                        BL::err('RSSTitle', 'Forum'),
                        BackendModel::createURLForAction('Settings', 'Forum')
                    )
                );
            }

            // rss description
            if (BackendModel::getModuleSetting('Forum', 'rss_description_' . BL::getWorkingLanguage(), null) == '') {
                $warnings[] = array(
                    'message' => sprintf(
                        BL::err('RSSDescription', 'Forum'),
                        BackendModel::createURLForAction('Settings', 'Forum')
                    )
                );
            }
    ...
    public static function delete($ids)
    {
        ...
        // delete tags
        foreach ($ids as $id) {
            BackendTagsModel::saveTags($id, '', 'Forum');
        }

        // invalidate the cache for forum
        BackendModel::invalidateFrontendCache('Forum', BL::getWorkingLanguage());
    }
    ...
    public static function deleteCategory($id)
    {
        ...
        if (!empty($item)) {
            ...
            // delete category
            $db->delete('forum_categories', 'id = ?', array($id));

            // update category for the posts that might be in this category
            $db->update('forum_posts', array('category_id' => null), 'category_id = ?', array($id));
    ...
    public static function deleteComments($ids)
    {
        ...
        // update record
        $db->delete('forum_comments', 'id IN (' . implode(', ', $idPlaceHolders) . ')', $ids);
    ...
    public static function deleteSpamComments()
    {
        ...
        // update record
        $db->delete('forum_comments', 'status = ? AND language = ?', array('spam', BL::getWorkingLanguage()));
    ...
    public static function getByTag($tagId)
    {
        $items = (array) BackendModel::getContainer()->get('database')->getRecords(
            'SELECT i.id AS url, i.title AS name, mt.module
             ...
             INNER JOIN forum_posts AS i ON mt.other_id = i.id
             WHERE mt.module = ? AND mt.tag_id = ? AND i.status = ? AND i.language = ?',
            array('Forum', (int) $tagId, 'active', BL::getWorkingLanguage())
        );

        // overwrite the url
        foreach ($items as &$row) {
            $row['url'] = BackendModel::createURLForAction('Edit', 'Forum', null, array('id' => $row['url']));
        }
        ...
        if ($includeCount) {
            return (array) $db->getPairs(
                'SELECT i.id, CONCAT(i.title, " (", COUNT(p.category_id) ,")") AS title
                 FROM forum_categories AS i
                 LEFT OUTER JOIN forum_posts AS p ON i.id = p.category_id AND i.language = p.language AND p.status = ?
                 WHERE i.language = ?
                 GROUP BY i.id',
    ...
    public static function getLatestComments($status, $limit = 10)
    {
        ...
        // overwrite url
        $baseUrl = BackendModel::getURLForBlock('Forum', 'detail');
    ...
    {
        return (int) BackendModel::getContainer()->get('database')->getVar('SELECT MAX(id) FROM forum_posts LIMIT 1');
    }
    ...
    public static function insertCompletePost($item, $meta = array(), $tags = array(), $comments = array())
    {
        ...
        if (!isset($item['profile_id'])) {
            $item['profile_id'] = BackendAuthentication::getUser()->getUserId();
        }
        ...
        // Any tags?
        if (!empty($tags)) {
            BackendTagsModel::saveTags($item['id'], implode(',', $tags), 'forum');
        }
    ...
    public static function insertCategory(array $item, $meta = null)
    {
        ...
        // create category
        $item['id'] = $db->insert('forum_categories', $item);
    ...
    /**
     * Inserts a new comment (Taken from FrontendForumModel)
     *
    ...
    public static function insertComment(array $comment)
    {
        ...
        // insert comment
        $comment['id'] = (int) $db->insert('forum_comments', $comment);
        ...
        if ($comment['status'] == 'published') {
            ...
            // update num comments
            $db->update('forum_posts', array('num_comments' => $numComments), 'id = ?', $comment['post_id']);
        }
    ...
    public static function updateCategory(array $item, $meta = null)
    {
        ...
        // update category
        $updated = $db->update('forum_categories', $item, 'id = ?', array((int) $item['id']));
    ...
    public static function updateCommentStatuses($ids, $status)
    {
        ...
        if (!empty($items)) {
            ...
            foreach ($languages as $language) {
                BackendModel::invalidateFrontendCache('Forum', $language);
            }
        }
    }
}
